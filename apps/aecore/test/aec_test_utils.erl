...
mock_fast_cuckoo_pow() ->
    mock_cuckoo_pow({"mean15-generic", "-t 5", 15, false, 10}).

mock_fast_and_deterministic_cuckoo_pow() ->
    mock_cuckoo_pow({"mean15-generic", "", 15, false, 10}).
...
mock_prebuilt_cuckoo_pow(MinerBin) ->
    ...
    mock_cuckoo_pow({MinerBin, "", 15, false, 10}).

mock_cuckoo_pow({_MinerBin, _MinerExtraArgs, _EdgeBits, _EncodedHeader, _Repeats} = Cfg) ->
